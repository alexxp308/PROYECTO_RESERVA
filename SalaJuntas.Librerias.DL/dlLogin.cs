...
namespace WebApplication1.Librerias.DL
{
    public class dlLogin
    {
		public string checkLogin(string userName, string password,SqlConnection con)
		{
			...
			if(drd!=null)
			{
				while (drd.Read())
				{
					result = drd.GetInt32(0) + "|" + drd.GetString(1)+"|"+drd.GetString(2)+"|"+ drd.GetInt32(3) + "|" + drd.GetInt32(4) + "|" + drd.GetString(5)+"|"+drd.GetString(6);
				}
				drd.Close();
			}
