(function ()
{
    ...
    var rs = (function (o)
    {
        ...
        o.formatoCheck = function(activos)
        {
            var nuevoCheck = {};
            nuevoCheck["ticket"] = "";
            nuevoCheck["detalle_sala"] = "";
            nuevoCheck["activos"] = {};
            var keys = Object.keys(activos);
            for (var j = 0; j < keys.length; j++)
            {
                nuevoCheck["activos"][keys[j]] = { descripcion: "", img: "", Detalle: [] };
                for (var z = 0; z < activos[keys[j]]; z++)
                {
                    nuevoCheck["activos"][keys[j]]["Detalle"].push("");
                }
            }
            return JSON.stringify(nuevoCheck);
        }
                ...
                for (var j = 0; j < o.myevents.length; j++)
                {
                    if (Math.trunc(o.myevents[j].id / 100) == 99999)
                    {
                        ...
                        o.nuevasReservas.push({//se guardan las posibles nuevas reservas
                            ...
                            checkListInicial: o.formatoCheck(obj),
                            checkListFinal: o.formatoCheck(obj),
                        });
